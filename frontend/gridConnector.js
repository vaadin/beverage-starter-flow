window.Vaadin.Flow.gridConnector = {
  initLazy: function(grid) {
    ...
    // Patch ItemCache prototype, but do not patch twice
    if (!window.Vaadin.Flow.gridConnector.cachePatched) {
      window.Vaadin.Flow.gridConnector.cachePatched = true;
      ...
      const cacheConstructor = Object.getPrototypeOf(grid._cache);

      cacheConstructor.ensureSubCacheForScaledIndex = function(scaledIndex) {
      ...
      cacheConstructor.doEnsureSubCacheForScaledIndex = function(scaledIndex) {
        if (!this.itemCaches[scaledIndex]) {
          const subCache = new cacheConstructor(this.grid, this, this.items[scaledIndex]);
      ...
      cacheConstructor.getCacheAndIndexByKey = function(key) {
      ...
      cacheConstructor.getLevel = function() {
        let cache = this;
        let level = 0;
    ...
    */
    ...
    grid.$connector.updateItem = grid._updateItem;

    grid._updateItem = function(row, item) {
      grid.$connector.updateItem.call(grid, row, item);
